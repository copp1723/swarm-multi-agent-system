...
import logging
from src.services.supermemory_service import SupermemoryService, MemoryQuery
from src.config import config
from src.exceptions import SwarmException, ValidationError, ServiceError
...
def memory_health():
    ...
    if not supermemory_service:
        return create_error_response(
            ServiceError("Supermemory service not initialized", "SERVICE_NOT_INITIALIZED"),
            503
        )
    ...
    try:
        ...
        return create_error_response(
            ServiceError("Memory health check failed", "HEALTH_CHECK_FAILED"),
            500
        )
...
def get_conversation_history(agent_id: str):
    ...
    try:
        ...
        return create_error_response(
            SwarmException("Internal server error", "INTERNAL_ERROR"),
            500
        )
...
def store_conversation(agent_id: str):
    ...
    try:
        ...
        if not user_message or not user_message.strip():
            raise ValidationError("user_message is required and cannot be empty", "MISSING_USER_MESSAGE")
        
        if not agent_response or not agent_response.strip():
            raise ValidationError("agent_response is required and cannot be empty", "MISSING_AGENT_RESPONSE")
        ...
        return jsonify(create_success_response({
            ...
            "agent_id": agent_id,
            "stored": True
        }, "Conversation stored successfully")), 201
...
def search_memory():
    ...
    try:
        ...
        if similarity_threshold < 0 or similarity_threshold > 1:
            raise ValidationError("similarity_threshold must be between 0 and 1", "INVALID_THRESHOLD")
    ...
    try:
        ...
        if success:
            ...
            return jsonify(create_success_response({
                "agent_id": agent_id,
                "cleared": False,
                "message": "No memory found to clear"
