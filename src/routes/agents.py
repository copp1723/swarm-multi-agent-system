...
from src.exceptions import SwarmException, ValidationError, AgentNotFoundError
...
def get_available_models():
    ...
    try:
        ...
        return jsonify(create_success_response(
            {"models": model_list},
            f"Retrieved {len(model_list)} models"
        ))
...
def collaborate_with_agents():
    ...
    try:
        ...
        invalid_agents = [agent_id for agent_id in mentioned_agents if agent_id not in valid_agent_ids]
        if invalid_agents:
            raise ValidationError(
...
def clear_agent_memory_route(agent_id: str):
    ...
    try:
        ...
        return jsonify(create_success_response({
            "agent_id": agent_id,
            "cleared": success
        }, f"Memory cleared for agent {agent_id}" if success else f"No memory found for agent {agent_id}"))
