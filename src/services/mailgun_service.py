...
class MailgunService(BaseService):
    ...
    def __init__(self, api_key: str, domain: str, webhook_signing_key: str = None, 
...
The Swarm Team
                """.strip(),
                variables=["name", "task_title", "agent_name", "start_time", "completion_time", "duration", "summary"],
...
The Swarm Team
    ...
    @handle_service_errors
    def send_template_email(self, template_name: str, recipients: List[str], 
                           variables: Dict[str, str], agent_id: str = None) -> Dict[str, Any]:
    ...
    def compose_ai_email(self, context: Dict[str, Any], agent_id: str) -> EmailMessage:
        ...
        if not recipient:
            raise ServiceError(
                "Recipient is required for AI email composition",
                error_code="MISSING_RECIPIENT"
            )
        
        if not purpose:
            raise ServiceError(
                "Email purpose is required for AI email composition",
                error_code="MISSING_PURPOSE"
            )
...
<p>{closing.replace(chr(10), '<br>')}</p>
    ...
    def process_webhook(self, webhook_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        message_id = webhook_data.get("event-data", {}).get("message", {}).get("headers", {}).get("message-id")
        ...
        if event_type == "delivered":
            ...
            reason = webhook_data.get("event-data", {}).get("delivery-status", {}).get("description")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if total_stats["sent"] > 0:
                    total_stats["delivery_rate"] = (total_stats["delivered"] / total_stats["sent"]) * 100
                    total_stats["bounce_rate"] = (total_stats["bounced"] / total_stats["sent"]) * 100
