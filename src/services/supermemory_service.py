...
class SupermemoryService(BaseService):
    ...
    def store_conversation(self, agent_id: str, user_message: str, agent_response: str, 
                          model_used: str = None, metadata: Dict[str, Any] = None) -> str:
...
"""
    ...
    def get_conversation_history(self, agent_id: str, limit: int = 20) -> List[ConversationEntry]:
        ...
        try:
            ...
            response = self.post(f"{self.base_url}/api/search", json=query_payload, headers=self.headers)
    ...
    def search_memory(self, query: MemoryQuery) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Found {len(memory_items)} relevant memory items for query: {query.query}")
                return memory_items
    ...
    def get_agent_context(self, agent_id: str, current_message: str, context_limit: int = 5) -> str:
        ...
        context = "\n".join(context_parts)
        logger.info(f"Generated context for agent {agent_id} with {len(memory_items)} relevant items")
        return context
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for item_id in items_to_delete:
                    try:
                        delete_response = self.delete(f"{self.base_url}/api/delete/{item_id}", headers=self.headers)
